...
class MinHeap:
    ...
    def add(self, value): # Add a value to the heap and maintain min-heap order
        self.data.append(value)
        self.heapify_up()

    def get(self): # Remove and return the smallest value (min root) from the heap
        if self.is_empty():
            return None
        minimum = self.data[0]
    ...
    def heapify_up(self):
        ...
        while pos > 0:
            ...
            if self.data[pos][0] < self.data[parent][0]:  # compare by distance
    ...
    def heapify_down(self):
        ...
        while min_child is not None:
            if self.data[pos][0] > self.data[min_child][0]:
                self.data[pos], self.data[min_child] = self.data[min_child], self.data[pos]
                pos = min_child
                min_child = self.get_min_child(pos)
            else:
                break
        ...
        while not heap.is_empty():
            ...
            for e in u.edges:
                ...
                if e.connection.key not in visited:
                    heap.add((e.connection.distance, e.connection))
